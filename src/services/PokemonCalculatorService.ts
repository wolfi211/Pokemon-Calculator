...
export class PokemonCalculatorService {
  ...
  private calculateEffectiveness(attackTypeId: number, defenderTypeIds: number[]): number {
    return defenderTypeIds.reduce((acc, defId) => acc * (TYPE_CHART[attackTypeId]?.[defId] ?? 1), 1);
  }
